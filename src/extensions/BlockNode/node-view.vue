<template>
  <node-view-wrapper class="umo-node-view" :data-node-id="props.node.attrs.nodeId">
    <div class="umo-node-paragraph-action" v-show="showToolbar">
      <div class="umo-node-paragraph-action-item" v-for="item in btns" :key="item.value" @mousedown.prevent
        @click="onClickBtn(item.value)">{{ item.label }}</div>
    </div>
...
import { MessageEnum, MessageSourceEnum, sendMessageToMain, SendMessageToMainParamsType } from '@/qiankun'
